...
int n;
    ...
    public int numWays(int n, int[][] relation, int k) {
        ...
        for (int i = 0; i < relation.length; i++) {
            List<Integer> tempList = data.get(relation[i][0]);
            tempList.add(relation[i][1]);
    ...
    private void search(int start, int times) {
        System.out.println(start + "\t" + times);
        if (times < 0) {
            return;
        }
        List<Integer> searchList = data.get(start);
        for (Integer oneInt : searchList) {
            if (times == 0 && oneInt == (n - 1)) {
                System.out.println(start);
                count++;
            }
            search(oneInt, times - 1);
        }
    }
}
